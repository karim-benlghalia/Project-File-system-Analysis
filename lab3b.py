...
class dirent:
    def __init__(self,argument):
        ...
        self.len_ent             =int(argument[4])
        self.len_name            =int(argument[5])
...
def check_inode_alloc(inode):
    ...
    if inode.Inode_Numb != 0:
        ...
        if inode.Inode_Numb in IFREE_argument:
            ...
            inode_bitmap.remove(inode.Inode_Numb)
...
def check_directory_block():
    ...
    max_inode = superblock_argument.s_inodes_count
    inode_par_array = {}
    ...
    for every_inode in inode_argument:
        ...
        for direct in dirent_argument:
            ...
            elif every_inode.Inode_Numb == direct.Inode_Numb:
                if(direct.inode < 1 or direct.inode > max_inode): #report INVALID
                    print("DIRECTORY INODE %d NAME %s INVALID INODE %d" %(direct.Inode_Numb, direct.name, direct.inode))
                elif direct.inode in inode_bitmap:
                        print("DIRECTORY INODE %d NAME %s UNALLOCATED INODE %d" %(direct.Inode_Numb, direct.name, direct.inode))
            if direct.name == "'.'" and direct.inode != direct.Inode_Numb:
                print("DIRECTORY INODE %d NAME %s LINK TO INODE %d SHOULD BE %d" %(direct.Inode_Numb, direct.name, direct.inode, direct.Inode_Numb))
            if direct.name != "'.'" and direct.name != "'..'":
                inode_par_array[direct.inode]=direct.Inode_Numb
        if link_count != every_inode.i_Links_Count:
            print("INODE %d HAS %d LINKS BUT LINKCOUNT IS %d" %(every_inode.Inode_Numb, link_count, every_inode.i_Links_Count))
    for direct in dirent_argument:
        if direct.name == "'..'" and inode_par_array[direct.Inode_Numb] != direct.inode:
            print("DIRECTORY INODE %d NAME %s LINK TO INODE %d SHOULD BE %d" %(direct.Inode_Numb, direct.name,direct.inode, inode_par_array[direct.Inode_Numb]))
